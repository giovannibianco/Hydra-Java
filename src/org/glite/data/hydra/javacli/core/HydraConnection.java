...
public class HydraConnection {
	 ...
	 */
    private void createEntry(File filename, String userid, String schema) throws ExistsException, RemoteException, ServiceException, NoSuchAlgorithmException {
		...
		String file_entry = getEntryName(filename, userid);
                // System.out.println("createEntry:  attempting to insert file entry: " + file_entry);
		StringPair[] entries = {new StringPair(file_entry, this.schema)};
                // System.out.println("createEntry:  attempting to insert entry: " + Arrays.deepToString(entries));
		this.mdcsl.getMetadataCatalog().createEntry(entries);
		    /** so the entry name already exists! So we should generate another entry name and re-try?
                          Maybe not... first try passing the exception back out and handling in the GUI?  */
	}
	
    private void createEntry(String file_entry, String schema) throws ExistsException, RemoteException, ServiceException, NoSuchAlgorithmException {
	        // System.out.println("createEntry:  attempting to insert file entry: " + file_entry);
		StringPair[] entries = {new StringPair(file_entry, this.schema)};
                // System.out.println("createEntry:  attempting to insert entry: " + Arrays.deepToString(entries));
		this.mdcsl.getMetadataCatalog().createEntry(entries);
	...
	private void removeEntry(File filename, String userid) throws NoSuchAlgorithmException, AuthorizationException, NotExistsException, InternalException, RemoteException, ServiceException {
		String[] file_entry = {getEntryName(filename, userid)};
                // System.out.println("removeEntry:  attempting to remove entry: " + Arrays.toString(file_entry));
	...
	public static String getEntryName(File filename, String userid) throws NoSuchAlgorithmException {
	    ...
	    // String file_entry = filename.getName() + "" + userid + "" + HydraUtils.bytesToString(getHash(filename, userid)); // here is an interesting bit. JSW.
	    String file_entry = filename.getName() + "XX" + HydraUtils.bytesToString(getHash(filename));
	    // String file_entry = filename.getName() + ""  + UUIDString;
            // System.out.println("getEntryName: file entry is: " + filename.getName() + "XX" + HydraUtils.bytesToString(getHash(filename)));
	    return file_entry;
	}
	 ...
	 * @throws RemoteException 
	 */
    public static void createEntries(HydraConnection[] connections, File filename, String userid, String schema) throws ExistsException, RemoteException, NoSuchAlgorithmException, ServiceException {
		if (schema == null) {
			schema = "eds";
		}
		for (int i = 0; i < connections.length; i++) { 
                        // connections[i].createEntry(filename, userid, schema);
			try{
                             connections[i].createEntry(filename, userid, schema);
			}
                        catch(AxisFault af){
                            if (af.dumpToString().contains("ExistsException")) {
                              System.out.println("createEntries: File " + filename + " already exists on Hydra connection: " + connections[i]);
			    }
                        }
		}
	}
		...
		for (int i = 0; i < connections.length; i++) { 
			try {
			    ...
			    connections[i].removeEntry(filename, userid);
                            ...
                            if (af.dumpToString().contains("InternalException")) {
				System.out.println("forceRemoveEntries: InternalException");
			    }
                            if (af.dumpToString().contains("AuthorizationException")) {
				System.out.println("forceRemoveEntries: AuthorizationException");
			    }
                            if (af.dumpToString().contains("NotExistsException")) {
                              System.out.println("forceRemoveEntries: File " + filename + " does NOT exist on Hydra connection: " + connections[i]);
			    } else {
                                if (af.dumpToString().contains("ExistsException")) {
                                    System.out.println("forceRemoveEntries: File " + filename + " DOES exist on Hydra connection: " + connections[i]);
			        }
                            }
                        } catch (NoSuchAlgorithmException e){
                          System.out.println("forceRemoveEntries: NoSuchAlgorithmException");
                        } catch (RemoteException e){
                          System.out.println("forceRemoveEntries: RemoteException");
                       } catch (ServiceException e){
                          System.out.println("forceRemoveEntries: ServiceExceptionn");
			}
                    }
               }
	...
	public static HydraKey gatherKey(
		...
		System.out.format("gatherKey: Required key slices... %n",required_keyslices );
	...
	private static byte[] getHash(File filename, String userid) throws NoSuchAlgorithmException {
		...
		byte[] hash = md.digest((filename.toString() + userid).getBytes());
                // System.out.println("getHash: hash returned: " + Arrays.toString(hash));
		return hash;
	}

	private static byte[] getHash(File filename) throws NoSuchAlgorithmException {
		MessageDigest md = MessageDigest.getInstance("SHA");
		byte[] hash = md.digest((filename.toString()).getBytes());
                // System.out.println("getHash: hash returned: " + Arrays.toString(hash));
		return hash;
	}
