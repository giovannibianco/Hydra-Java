...
public class Gui {
	...
	private void initialize() {
		...
		frmJhydraClientGui.setSize(new Dimension(580, 320));
		frmJhydraClientGui.setTitle("JSW JHydra Client 0.1");
		...
		btnCryptdecrypt.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					// first, read the certificates JSW need to check from here onwards!
				...
				if (hk != null && decrypt && hs != null) {
					...
					try {
						...
						try{
						    ...
						    // System.out.println("XXX JSW: Axis fault: " + af.dumpToString());
                 					if (af.dumpToString().contains("ExistsException")) {
							    System.out.println("XXX JSW: the target file " + targetFile + " already exists!"); // what does this mean?
							}
						}
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					settingsXmlField.setText(settingsXmlFile.getPath());
                                        // System.out.format("XML file %s",settingsXmlFile );
				} catch (Exception e1) {
					// TODO Auto-generated catch block
					writeToGuiLog(e1.getMessage());
				}
		...
		JLabel lblServerCertificate = new JLabel("Certificate Truststore");
